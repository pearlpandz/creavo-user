...
import MoreColorPicker from "../components/MoreColorPicker";
...
export default function Editor() {
  ...
  const {
    ...
    isFetching1,
    isRefetching1,
  } = useTemplates(selectedCategory?.name, {
  ...
  const { data: selectedTemplateDetail } = useTemplateDetail(
  ...
  const productDetails = useMemo(() => {
    return <ProductInfo productList={profile?.products} isEditorView={true} />;
  }, [profile?.products]);

  const politicalDetails = useMemo(() => {
    return <PoliticalDetails detail={profile?.political} isEditorView={true} />;
  }, [profile?.political]);
  ...
  return (
    <>
      <div className="editor-body">
        ...
        <div className="picker-container">{currentSidebarElement}</div>
...
const FramesContainer = ({
  ...
  const selectedCateogoryItems = templates?.filter(
          ...
          {backgroundColors.map((color) => (
            <Box
              ...
              sx={{
                ...
                background: color,
                border:
                  selectedTheme?.bgColor === color
                    ? "2px solid #333"
                    : "2px solid #ccc",
                cursor: "pointer",
                boxShadow:
                  selectedTheme?.bgColor === color ? "0 0 4px #333" : "none",
              }}
          ...
          {textColors.map((color) => (
            <Box
              ...
              sx={{
                ...
                background: color,
                border:
                  selectedTheme?.textColor === color
                    ? "2px solid #333"
                    : "2px solid #ccc",
                cursor: "pointer",
                boxShadow:
                  selectedTheme?.textColor === color ? "0 0 4px #333" : "none",
              }}
      ...
      <Box>
        ...
        <Stack direction="column" spacing={2}>
          ...
          <Button
            ...
            component="label"
            size="small"
            sx={{ fontSize: 12, width: 150, height: 30 }}
          >
            Upload Image
            <input
              type="file"
