...
export default function PickLanguage({ limit = 1 }) {
    ...
    useEffect(() => {
        axios.get(SETTINGS.DJANGO_URL + '/api/categories/signup-list/').then((res) => {
            if (res?.data?.length > 0) {
                const businessCategories = res.data.find(a => a.name?.toLowerCase().includes('language'))
    ...
    const handleContinue = async () => {
        ...
        const payload = {
            language: selectedCategory?.map(category => category.id)
        }
        try {
